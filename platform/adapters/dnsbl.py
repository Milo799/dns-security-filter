...
class DroneBLAdapter(DNSBLAdapter):
    ...
    .10（广告追踪）/ .11（IRC 滥用）默认忽略——广告追踪属商业属性
    标注而非恶意主机，IRC 滥用多为聊天服务器行为，两者计入拦截会
    误伤浏览类正常流量（与 ZEN 的 PBL 忽略同理）。
    """
    name = "dronebl"
...
class SPFBLAdapter(DNSBLAdapter):
    """SPFBL：邮件信誉评分清单（评分信号，非恶意主机清单）。

    官方语义（spfbl.net）：
      127.0.0.2 = 确认的垃圾邮件发送源
      127.0.0.3 = 疑似垃圾（评分信号，MTA 不合规）
      127.0.0.4 = NAT/住宅/非邮件服务器
      127.0.0.5 = abuse 处理不可靠
    官方自述"仅作评分信号，不应在 SMTP 事务中直接拒收"。浏览类 DNS
    过滤中仅 .2（确认垃圾）计入拦截，.3/.4/.5 属弱信号/名单属性标注，
    直接拦截会误伤大量正常站点，默认忽略。
    """
    name = "spfbl"
    ...
    supports_domain = True
    supports_ip = True
    # 仅 127.0.0.2（确认垃圾）计恶意；.3/.4/.5 弱信号默认忽略——
    # 现有实现曾把 .3 错标"恶意软件"、.4 错标"钓鱼"、.5 错标"僵尸
    # 网络"且全码拦截，属语义错配（2026-09-03 评估确认并修正）
    _IGNORE_CODES = {"127.0.0.3", "127.0.0.4", "127.0.0.5"}
    code_map = {
        "127.0.0.2": "确认垃圾邮件发送源",
        "127.0.0.3": "疑似垃圾（评分信号）",
        "127.0.0.4": "NAT/住宅/非邮件服务器",
