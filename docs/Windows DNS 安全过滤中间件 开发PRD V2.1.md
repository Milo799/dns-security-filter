...
**★ 生产稳定性加固（迭代 42）**：针对 2026-09-10 生产事故（spamhaus_dbl 在线源全天 5006 次超时 → 每个未命中缓存域名卡 5s → executor 线程全被慢查询占满 → asyncio 无限队列积压数千查询 → 响应延迟远超 proxy 8s 转发超时 → 全网"解析不了"）的四项加固：①**DNSBL 查询超时上限**（dnsbl_max_timeout_ms 默认 2500ms，热生效——DNSBLAdapter._effective_timeout_s 取源级 timeout_ms 与全局上限的小值，源级 5000ms 被压到 2.5s，0=不限恢复旧行为；与源级熔断配合把故障期单源代价限制在冷却后半开探测）②**检测队列深度上限**（max_queue_depth 默认 500，热生效——queue_stats.submitted() 在 pending 达上限时返回 False 并计 rejected，dns_server UDP/TCP 双路径立即回 SERVFAIL 快速失败不提交 executor；客户端按自身重试策略自愈，把故障期表现从"全网瘫"收敛为"部分查询短暂失败"；/api/queue-stats 增 rejected 计数）③**多上游重试**（upstream_dns_backup 逗号分隔备用列表，默认空=禁用——query_upstream/query_upstream_reply 主上游失败后依次尝试备用，任一成功即返回并计熔断成功（服务视角健康），全部失败才计一次熔断失败；总耗时 =(1+备份数)×upstream_timeout_s 须 < proxy forward_timeout 8s，建议只配 1 个）④**名单导入分批短事务**（import_source 三段式：建 staging 表→分批 INSERT 每批独立事务→原子换入（DELETE+INSERT...SELECT+DROP 单事务纯 SQLite 内部搬运）——原单事务 DELETE+全量 INSERT 锁库 1~3 分钟，期间 query_stats/log_writer 落库反复撞 "database is locked"；staging 用普通表（db.py 线程本地连接 TEMP 表互不可见），遗留 staging 由下次导入 DROP 重建自愈）。前端：系统配置页"上游与日志"卡增备用上游输入框 + 新增"过载保护"卡（DNSBL 超时上限/队列深度上限）；PUT /api/config 三键校验（超时 0~30000ms/队列 0~100000/备用 ≤3 个逐项 ip[:port] 正则）。484 项测试全绿

**★ 银狐情报共享站离线源（迭代 43）**：首个**API 拉取型**内置离线源 `silverfox`——微步在线"银狐"木马情报共享站（s.threatbook.com/cybercrime/silverfox，专打中文用户的远控家族：仿冒国产软件官网投毒/钓鱼诱饵/C2，2023-06 至今约 1175 个事件、域名+IP 约 2200 条，免登录免 Key 无配额）。与既有 9 个"单 URL 文本下载型"源不同，数据经两步聚合（app/silverfox.py）：`get-humans` 按 92 天窗口回溯拉事件 uuid 列表 → `get-human?uuid` 逐事件提取 related_ioc_list_v2（domain/ioc/ip/url 四类；hash 丢弃，URL 提取 host 并入域名）+ `get-hot-ioc` 每日热点防御性合并；0.3s/请求限速（全量约 6 分钟，后台任务化）。框架扩展：SOURCES 元数据新增 `"api"` 键（无 url），下载/导入链路按该键分流——路由 `_run_import_task` 与 `auto_update_once` 走 `fetch_iocs()` + **`import_api_source`（域名→target=domain 走主检测链 4.5 段、IP→target=ip 走 PTR 反查 find_ip 拦截，双 target 整源替换）**；staging 三段式从 import_source 抽出为 `_replace_source_rows` 复用。防护：空结果拒绝导入（整源替换不清旧库）、连续 30 事件拉取失败熔断中止（非官方 XHR 接口可能改版）、拉取失败保留旧数据 fail-safe；出站走 http_client 共享 Client（生产自动经内网代理）。配置键 `silverfox_window_days`（热生效，0=全量回溯至 2023-06"一个不漏"，N>0 滚动窗口——窗口外旧事件 IOC 整源淘汰，控制老域名被正常方接管后的误拦风险）；离线情报源页自动更新设置行增"银狐回溯"输入框，导入进度条 download 阶段优先显示 message（API 源无字节数）。reason=threat_list:silverfox，前端源管理页/条目查看/审计零改动自动支持。**API 源周期保护**：auto_update_once/next_update_schedule 中 API 拉取型源不受全局间隔缩短（生效周期恒为源自身 24h）——每轮拉取都是全量重拉（1175 事件×0.3s≈6 分钟+上千请求），生产全局间隔 1h 若照旧 min() 缩短会每小时重打共享站；文件下载型源照旧可被全局间隔缩短。501 项测试全绿（test_silverfox.py 17 项）

**★ 银狐数据归属统一（迭代 43.1，2026-09-17）**：银狐数据从人工黑名单整体迁入离线源 silverfox 名单（用户拍板"所有数据+逻辑一起移过去"）。两项修正：①**URL host IP 双收集**——迁移差集分析发现 91 个投毒下载源 IP（http://IP/payload.exe 形态）仅存在于事件 url/ioc 字典的 URL 里，`_url_hosts` 原只收域名型 host 会整批漏掉，改为域名与 IP 双收集（找回后 silverfox 源 IP 776→867）；②**本地追加文件 `platform/data/silverfox_extra.txt`**（与 platform.db 同目录，每行一条经 classify 分流）——承载站点已下架条目与注册域兜底（如 vuxu661d.com 裸域，threat_list 父域匹配等效 \*.vuxu661d.com 通配且更强；threat_list 不支持通配值，人工名单的通配条目以此形态迁入），随每轮 fetch 合入、不随整源替换丢失；不存在视为空。人工黑名单清理：09-15 手工 62 条 + 09-16 应急导入 2085 条（remark 含"银狐"或 created_at=09-15）共 2147 条删除，备份先行（CSV 留档），删除后 touch updated_at 触发 cross_sync 缓存失效。503 项测试全绿（test_silverfox.py 19 项）
