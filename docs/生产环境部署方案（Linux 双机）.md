...
**关于 Web 管理界面的 HTTPS**：平台自身只提供 HTTP（8080），**不支持、也不建议**在
uvicorn 上直接启用 TLS。生产要求 8080 仅限运维网段/堡垒机可达（上联防火墙/ACL 实施），
管理流量走内网明文可接受；若安全基线强制要求 HTTPS，请在机 B 前置 Nginx 反向代理
（`listen 443 ssl` + `proxy_pass http://127.0.0.1:8080`，证书由企业 CA 签发），
防火墙同步放行 443、收敛 8080 至仅本机/运维段——平台配置无需任何改动。
