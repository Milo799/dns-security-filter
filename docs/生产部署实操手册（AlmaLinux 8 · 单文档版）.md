...
> **HTTPS 说明**：Web 管理界面平台自身仅提供 HTTP，不支持在 uvicorn 上直接启用 TLS。
> 当前方案依赖网络层隔离（8080 仅运维网段/堡垒机可达）承载管理流量；若安全基线
> 强制 HTTPS，在机 B 前置 Nginx 反向代理（443 ssl → 127.0.0.1:8080，企业 CA 证书），
> 防火墙放行 443、8080 收敛至本机，平台配置无需改动。
