...
| `platform/` | 平台：dns_server（ECS/PTR）、detectors 五层检测、adapters（16 适配器）、threat_list（离线大名单）、rdap_nrd（NRD 新注册域名检测：RDAP 注册时间查询+永久缓存）、domain_cache/ip_cache（结论缓存）、circuit_breaker（熔断：源级+路径级+上游熔断 fast-fail）、log_writer（日志削峰）、log_retention（保留期清理）、cross_sync（跨进程同步）、queue_stats（线程池队列观测）、query_stats（今日请求全量统计）、app(FastAPI+SQLite+crypto)、seed | ✅ 已实现 |
...
| `tests/` | pytest 440 项（融合/拦截/ECS/PTR/大名单/情报源/调度/性能/缓存/并行/跨进程同步/线程池复用/上游熔断/队列观测/查询统计/认证安全/域名聚合分析/原因筛选下拉/NRD 新注册域名检测） | ✅ 全绿 |
...
- 离线大名单：整源替换导入、内存缓存匹配；写路径（导入/启停/清空）必须调 `invalidate()` 联动失效内存缓存与统计缓存；NRD 源（hagezi_nrd，迭代 41）命中走 nrd_offline_* 语义（observe/intercept），其余源命中即拦截（threat_list:\<key\>）；大文件下载双闸截断防护（Content-Length 校验 + NRD 文件头条数校验）；导入走 staging 表三段式分批短事务（迭代 42，防长事务锁库）；**API 拉取型源**（silverfox 银狐情报共享站，迭代 43）无 url、SOURCES 带 "api" 键分流到 `app/silverfox.py` fetch_iocs 两步聚合（事件列表→逐事件 IOC），`import_api_source` 双 target 导入（域名走主检测链、IP 走 PTR 反查链），回溯窗口 silverfox_window_days 热配置（0=全量回溯至 2023-06）；空结果拒绝导入、连续 30 事件失败熔断、失败保留旧数据 fail-safe；URL host 域名与 IP 双收集（迭代 43.1，09-17 归因 91 个投毒下载源 IP 仅存于 url 字典）；本地追加文件 `platform/data/silverfox_extra.txt`（每行一条经 classify 分流，站点已下架条目/注册域兜底常驻，随每轮导入合入不随整源替换丢失）
- **人工名单与离线源归属约定（迭代 43.1）**：单一情报源的数据统一归其离线源名单（如银狐数据全量在 threat_list silverfox 源），人工黑白名单只放用户手工维护的条目——需要通配形态的银狐注册域用追加文件放裸域（父域匹配等效 \*. 通配）
